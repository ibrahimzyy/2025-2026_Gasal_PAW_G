...
if ($id != "") {
    $q = $koneksi->query("SELECT * FROM supplier WHERE id='$id'");
    $data = $q->fetch_assoc();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id == "") {
        $koneksi->query("INSERT INTO supplier (nama, alamat, telp) VALUES ('$nama', '$alamat', '$telp')");
        echo "<script>alert('Supplier berhasil ditambahkan'); location.href='supplier.php';</script>";
    } else {
        $koneksi->query("UPDATE supplier SET nama='$nama', alamat='$alamat', telp='$telp' WHERE id='$id'");
        echo "<script>alert('Supplier berhasil diupdate'); location.href='supplier.php';</script>";
    }
}
?>
<!DOCTYPE html>
...
<div class="box">
    ...
    <form method="post">
        <input type="text" name="nama" placeholder="Nama Supplier" value="<?= $nama ?>" required>
